...
const Layout = ({ children, }: Readonly <{ children: React.ReactNode }>) => {
  ...
  return (
    <div className={`max-w-screen-xl max-h-screen-2xl font-afacad relative`}>
      <nav key={patname} className={`h-10 w-screen flex justify-end items-center gap-10 tracking-widest text-xs fixed top-[6rem] right-36  z-50  ${patname === "/navegacion" ? "animate-fade-in" : "" }`}>
